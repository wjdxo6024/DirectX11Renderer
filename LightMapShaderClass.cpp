...
// Private Method
bool LightMapShaderClass::InitShader(ID3D11Device* device, HWND hWnd, WCHAR* vs, WCHAR* ps)
{
	ID3D10Blob* errorMessage = nullptr;

	ID3D10Blob* vertexShaderBuffer = nullptr;
	if (FAILED(D3DCompileFromFile(vs, NULL, NULL, "LightMapVertexShader", "vs_5_0",
	...
	if (FAILED(D3DCompileFromFile(ps, NULL, NULL, "LightMapPixelShader", "ps_5_0",
...
bool LightMapShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX world, XMMATRIX view, XMMATRIX proj, ID3D11ShaderResourceView** textureArray)
{
...
bool LightMapShaderClass::Init(ID3D11Device* device, HWND hWnd)
{
	WCHAR vs[] = L"./Shader/lightmap.vs";
	WCHAR ps[] = L"./Shader/lightmap.ps";
	// TODO : 20210312 0153 여기까지 함, FBX Animation으로 인한 
	return InitShader(device, hWnd, vs, ps);
}

void LightMapShaderClass::Quit()
{
	Shutdown();
}

bool LightMapShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX world, XMMATRIX view, XMMATRIX proj, ID3D11ShaderResourceView** textureArray)
{
